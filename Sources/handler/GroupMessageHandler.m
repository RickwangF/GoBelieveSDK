...
-(BOOL)handleMessages:(NSArray *)msgs {
    ...
    for (IMMessage *im in msgs) {
        ...
        if (im.isSelf) {
            ...
            int msgId = [[GroupMessageDB instance] gobelieveGetMessageId:revoke.msgid];
            if (msgId > 0) {
                [[GroupMessageDB instance] gobelieveUpdateMessageContent:msgId content:im.content];
                [[GroupMessageDB instance] gobelieveRemoveMessageIndex:msgId];
            }
        } else {
            [imsgs addObject:m];
    ...
    if (imsgs.count > 0) {
        [[GroupMessageDB instance] gobelieveInsertMessages:imsgs];
    }
...
-(BOOL)handleMessageACK:(IMMessage*)msg error:(int)error {
    if (error == MSG_ACK_SUCCESS) {
        if (msg.msgLocalID > 0) {
            return [[GroupMessageDB instance] gobelieveAcknowledgeMessage:(int)msg.msgLocalID];
        } else {
            ...
            if (content.type == MESSAGE_REVOKE) {
                ...
                int revokedMsgId = [[GroupMessageDB instance] gobelieveGetMessageId:revoke.msgid];
                if (revokedMsgId > 0) {
                    [[GroupMessageDB instance]  gobelieveUpdateMessageContent:revokedMsgId content:msg.content];
                    [[GroupMessageDB instance] gobelieveRemoveMessageIndex:revokedMsgId];
                }
            }
            return YES;
        }
        ...
        if (msg.msgLocalID > 0) {
            return [[GroupMessageDB instance] gobelieveMarkMessageFailure:(int)msg.msgLocalID];
        }
        return YES;
    }
}
