...
NSString* stringOrEmpty(NSString* value) {
...
+ (SQLConversationDB*)sharedInstance {
...
- (void)parseConversationList:(FMResultSet*)rs
                   completion:(void (^ _Nullable)(NSArray<Conversation*>*))completion {
    if (!rs) {
...
- (void)manualCheckConversationDBColumn {
    ...
    [queue inTransaction:^(FMDatabase* db, BOOL* rollback) {
        ...
        for (int j = 0; j < columnNames.count; j++) {
            ...
            [all enumerateKeysAndObjectsUsingBlock:^(NSString* key, NSNumber* obj, BOOL* stop) {
