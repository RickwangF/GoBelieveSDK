    ...
    *p++ = (uint8_t)self.flag;
    ...
    if (self.cmd == MSG_HEARTBEAT || self.cmd == MSG_PING) {
        ...
        *p++ = strlen(t);
        ...
        memcpy(p, s, l);
        return [NSData dataWithBytes:buf length:HEAD_SIZE + 32 + l];
        ...
        p += 4;
        writeInt32(ack.status, p);
        p += 4;
        writeInt64(ack.timestamp, p);
        p += 8;
...
-(BOOL)unpack:(NSData*)data {
    ...
    if (self.cmd == MSG_PONG) {
        ...
        p += 8;
        m.timestamp = readInt64(p);
        p += 8;
        m.msgLocalID = 0;
        p += 8;
        m.content = [[NSString alloc] initWithBytes:p length:data.length - 40 encoding:NSUTF8StringEncoding];
        self.body = m;
        return YES;
    } else if (self.cmd == MSG_CUSTOMER || self.cmd == MSG_CUSTOMER_SUPPORT) {
        ...
        ack.seq = readInt32(p);
        p += 4;
        ack.status = readInt32(p);
        p += 4;
        ack.timestamp = readInt64(p);
        p += 8;
//        ack.status = *p;
        self.body = ack;
        return YES;
