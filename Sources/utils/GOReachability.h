...
typedef NS_ENUM(NSInteger, GBNetworkStatus) {
    // Apple NetworkStatus Compatible Names.
    GBNotReachable = 0,
    GBReachableViaWiFi = 2,
    GBReachableViaWWAN = 1
};

@class GOReachability;

typedef void (^GBNetworkReachable)(GOReachability * reachability);
typedef void (^GBNetworkUnreachable)(GOReachability * reachability);

@interface GOReachability : NSObject

@property (nonatomic, copy) GBNetworkReachable    reachableBlock;
@property (nonatomic, copy) GBNetworkUnreachable  unreachableBlock;
...
-(GBNetworkStatus)currentReachabilityStatus;
