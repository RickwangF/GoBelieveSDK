...
typedef NS_ENUM(NSUInteger, GBChatSendStatus) {
    GBChatSendStatusSuccess         = 0,//发送成功
    GBChatSendStatusSending         = 1,//发送中
    GBChatSendStatusFailed          = 2,//发送失败
};
...
@property(nonatomic) NSInteger msgLocalID;
...
@property(nonatomic, strong) IUser *senderInfo;

@property (nonatomic, strong) NSData *imageOriginData;
@property (nonatomic, strong) NSData *imageZipData;
@property (nonatomic, assign) GBChatSendStatus sendStatus;//发送状态（针对图片和语音）,正常发送的其他类型消息需要添加状态为success
@property (nonatomic, copy) NSString *mediaIden;
...
@property (nonatomic, assign) float manualWidth;
@property (nonatomic, assign) float manualHeight;
