...
- (SQLCustomerMessageIterator *)initWithDB:(FMDatabase *)db store:(int64_t)store {
    self = [super init];
    if (self) {
        NSString *sql = @"SELECT  id, customer_id, customer_appid, store_id, seller_id, timestamp, flags, is_support, "
                        @"content FROM customer_message WHERE store_id = ? ORDER BY id DESC";
...
- (SQLCustomerMessageIterator *)initWithDB:(FMDatabase *)db store:(int64_t)store position:(int)msgID {
    ...
    if (self) {
        NSString *sql = @"SELECT  id, customer_id, customer_appid, store_id, seller_id, timestamp, flags, is_support, "
                        @"content FROM customer_message WHERE store_id = ? AND id < ? ORDER BY id DESC";
...
- (SQLCustomerMessageIterator *)initWithDB:(FMDatabase *)db uid:(int64_t)uid appID:(int64_t)appID {
    ...
    if (self) {
        NSString *sql = @"SELECT  id, customer_id, customer_appid, store_id, seller_id, timestamp, flags, is_support, "
                        @"content FROM customer_message WHERE customer_id = ? AND customer_appid=? ORDER BY id DESC";
...
- (SQLCustomerMessageIterator *)initWithDB:(FMDatabase *)db uid:(int64_t)uid appID:(int64_t)appID position:(int)msgID {
    ...
    if (self) {
        NSString *sql =
            @"SELECT  id, customer_id, customer_appid, store_id, seller_id, timestamp, flags, is_support, content FROM "
            @"customer_message WHERE customer_id = ? AND customer_appid=? AND id < ? ORDER BY id DESC";
...
- (IMessage *)getLastMessage:(int64_t)uid appID:(int64_t)appID {
    FMResultSet *rs = [self.db
        executeQuery:@"SELECT id, customer_id, customer_appid, store_id, seller_id, timestamp, flags, is_support, "
                     @"content FROM customer_message WHERE customer_id= ? AND customer_appid=? ORDER BY id DESC",
                     @(uid), @(appID)];
    if ([rs next]) {
        ICustomerMessage *msg = [[ICustomerMessage alloc] init];
...
- (IMessage *)getLastMessage:(int64_t)storeID {
    FMResultSet *rs =
        [self.db executeQuery:@"SELECT id, customer_id, customer_appid, store_id, seller_id, timestamp, flags, "
                              @"is_support, content FROM customer_message WHERE store_id= ? ORDER BY id DESC",
                              @(storeID)];
    if ([rs next]) {
        ICustomerMessage *msg = [[ICustomerMessage alloc] init];
        msg.customerAppID = [rs longLongIntForColumn:@"customer_appid"];
...
- (IMessage *)getMessage:(int)msgID {
    FMResultSet *rs = [self.db executeQuery:@"SELECT id, customer_id, customer_appid, store_id, seller_id, timestamp, "
                                            @"flags, is_support, content FROM customer_message WHERE id= ?",
                                            @(msgID)];
    if ([rs next]) {
        ICustomerMessage *msg = [[ICustomerMessage alloc] init];
        msg.customerAppID = [rs longLongIntForColumn:@"customer_appid"];
...
- (BOOL)insertMessage:(IMessage *)msg {
    ...
    BOOL r = [db executeUpdate:sql, @(cm.customerID), @(cm.customerAppID), @(cm.storeID), @(cm.sellerID),
                               @(cm.timestamp), @(cm.flags), @(is_support), uuid, cm.rawContent];
    if (!r) {
